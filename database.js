...
async function classifyBug(id, classified, auth) {
	...
	if (selectedBug['classification']['classifiedOn'] == undefined) {
		...
		result = await db.collection("Bug").updateOne({ _id: newId(id) }, {
			$set: {
				"classification.classifiedAs": classified, "classification.lastUpdated": new Date(),
...
async function assignBug(userId, bugId, auth) {
	...
	const result = await db.collection("Bug").updateOne({ _id: newId(bugId) }, {
		$set: {
			assignedInfo: {
...
async function addCommentToBug(bugId, comment, auth) {
	...
	const result = db.collection("Comment").insertOne({
		text: comment.text, author: commentor.fullName,
...
async function addTestToBug(bugId, newTest, auth) {
	...
	const result = await db.collection("Testcase").insertOne({
		status: newTest.status, bug_id: bugId,
...
async function updateTestById(bugId, testId, updatedTest, auth) {
	...
	const result = await db.collection("Testcase").updateOne({ _id: newId(testId), bug_id: newId(bugId) },
		{
			$set: {
				status: updatedTest.status, "update.lastUpdated": new Date(),
...
async function getBugEditsByUserId(id) {
	const db = await connect();
	debugDb(id)
	const result = await db.collection("Edit").aggregate([
		{
			'$match': {
				'auth._id': id.toString(),
				'collection': 'Bug'
			}
		},
		{
...
export { createEdit, saveEdit, getBugEditsByUserId };
